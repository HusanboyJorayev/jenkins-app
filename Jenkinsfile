pipeline {
    ...
    environment {
        ...
        DEPLOY_SERVER = 'root@45.138.158.89'   // SSH user va server IP
        DEPLOY_PATH = '/root/'                 // Jar fayl saqlanadigan joy (oxirida slash)
        JAR_NAME = 'jenkins_app.jar'
        LOG_FILE = "/var/log/${JAR_NAME}.log"
        BACKUP_PATH = '/root/backup/'          // Backup joyi
    }
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sshagent(['agro_server_key']) {  // Jenkins Credentials ID
